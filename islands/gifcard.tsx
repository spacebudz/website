...
import { Card, CardHeader, CardTitle, CardContent, CardFooter } from "@/islands/ui/card/mod.tsx"; // adjust path as needed
    ...
    return (
      <Card className="w-full max-w-md mx-auto shadow-md rounded-lg">
        ...
        <CardContent className="flex justify-center items-center">
          {isLoading ? ( // Show loading message or spinner while fetching
            <p>Loading...</p>
          ) : gifUrl ? ( // Show the GIF when loaded
            <img
              src={gifUrl}
              alt="Random GIF"
        ...
        <CardFooter className="flex justify-center">
          <Button
            onClick={fetchGif}
            className={`bg-blue-500 hover:bg-blue-600 ${isLoading ? "cursor-not-allowed" : ""}`} // Disable the button during loading
            disabled={isLoading}
          >
            {isLoading ? "Loading..." : "Click Me"} {/* Change button text during loading */}
          </Button>
        </CardFooter>
      </Card>
    );
