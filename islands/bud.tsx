...
export function BudHeader({ children }: React.PropsWithChildren) {
  return (
    <a
...
export type BudBodyProps = {
  id: number;
  metadata: Metadata;
  asset: { policyId: string; assetName: string };
};

export function BudBody({ id, metadata, asset }: BudBodyProps) {
  ...
  const jsonMetadata = JSON.stringify(
    Object.fromEntries(
      Object.entries(metadata).filter(([key]) => key !== "extendedProperties"),
    ),
    null,
    2,
  );
              ...
              <Card className="w-full max-w-[300px] h-[100px] overflow-hidden relative border-none">
                <img
                  ...
                  className={cn(
                    ...
                    metadata.type === "Dino" ? "-top-[270px]" : "-top-72",
                  )}
                  src={src}
                />
              </Card>
...
function CopyButton(
  ...
  return (
    <Button
      ...
      variant="outline"
      onClick={() =>
        navigator.clipboard.writeText(data).then(() => setIsCopied(true))}
    >
      {isCopied
